...
import vscode_task_runner_old.terminal_task_system
from vscode_task_runner_old.exceptions import (
    DirectoryNotFound,
    FileNotFound,
    InvalidValue,
    UnsupportedValue,
...
class Task:
    ...
    def _get_options_setting(self, setting_key: str) -> Any:
        ...
        # global setting
        if (
            vscode_task_runner_old.constants.OPTIONS_KEY in self.all_task_data
            and setting_key
            in self.all_task_data[vscode_task_runner_old.constants.OPTIONS_KEY]
        ):
            value = self.all_task_data[vscode_task_runner_old.constants.OPTIONS_KEY][
                setting_key
            ]

        # global os-specific setting
        if (
            vscode_task_runner_old.constants.PLATFORM_KEY in self.all_task_data
            and vscode_task_runner_old.constants.OPTIONS_KEY
            in self.all_task_data[vscode_task_runner_old.constants.PLATFORM_KEY]
            and setting_key
            in self.all_task_data[vscode_task_runner_old.constants.PLATFORM_KEY][
                vscode_task_runner_old.constants.OPTIONS_KEY
            ]
        ):
            value = self.all_task_data[vscode_task_runner_old.constants.PLATFORM_KEY][
                vscode_task_runner_old.constants.OPTIONS_KEY
            ][setting_key]

        # task setting
        if (
            vscode_task_runner_old.constants.OPTIONS_KEY in self.task_data
            and setting_key
            in self.task_data[vscode_task_runner_old.constants.OPTIONS_KEY]
        ):
            value = self.task_data[vscode_task_runner_old.constants.OPTIONS_KEY][
                setting_key
            ]

        # task os-specific setting
        if (
            vscode_task_runner_old.constants.PLATFORM_KEY in self.task_data
            and vscode_task_runner_old.constants.OPTIONS_KEY
            in self.task_data[vscode_task_runner_old.constants.PLATFORM_KEY]
            and setting_key
            in self.task_data[vscode_task_runner_old.constants.PLATFORM_KEY][
                vscode_task_runner_old.constants.OPTIONS_KEY
            ]
        ):
            value = self.task_data[vscode_task_runner_old.constants.PLATFORM_KEY][
                vscode_task_runner_old.constants.OPTIONS_KEY
            ][setting_key]
    ...
    def env(self) -> Dict[str, str]:
        ...
        for key, value in env.items():
            env[key] = vscode_task_runner_old.helpers.stringify(value)
    ...
    def type_(self) -> TaskType:
        ...
        task_type = self._get_task_setting(
            "type", vscode_task_runner_old.constants.DEFAULT_TASK_TYPE.value
        )
    ...
    def _global_command(self) -> Union[CommandString, None]:
        ...
        if raw_global_command is not None:
            return vscode_task_runner_old.helpers.load_command_string(
                raw_global_command
            )
    ...
    def _task_command(self) -> Union[CommandString, None]:
        ...
        if raw_task_command is not None:
            return vscode_task_runner_old.helpers.load_command_string(raw_task_command)
    ...
    def args(self) -> List[CommandString]:
        ...
        final_args.extend(
            vscode_task_runner_old.helpers.load_command_string(arg)
    ...
    def shell(self) -> Tuple[ShellConfiguration, ShellType]:
        ...
        if not shell_configuration.executable:
            ...
            shell_configuration = vscode_task_runner_old.helpers.get_parent_shell()
        ...
        return shell_configuration, vscode_task_runner_old.helpers.identify_shell_type(
            shell_configuration.executable
        )
    ...
    def subprocess_command(self, extra_args: Optional[List[str]] = None) -> List[str]:
        ...
        if self.type_ == TaskType.process:
            ...
            vscode_task_runner_old.terminal_task_system.get_quoting_options(
            ...
            ] + vscode_task_runner_old.terminal_task_system.create_shell_launch_config(
                shell_type,
                shell_config.args,
                vscode_task_runner_old.terminal_task_system.build_shell_command_line(
                    shell_type,
