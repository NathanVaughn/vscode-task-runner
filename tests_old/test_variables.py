...
from vscode_task_runner_old.exceptions import UnsupportedValue, UnsupportedVariable
...
def test_get_input_value_env(environment_variable: None) -> None:
    ...
    """
    assert (
        vscode_task_runner_old.variables.get_input_value("test", [{"id": "test"}])
        == "abc123"
    )
...
def test_get_input_value_text(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch.object(vscode_task_runner_old.variables.questionary, attribute="text")

    vscode_task_runner_old.variables.get_input_value(
        "test", [{"id": "test", "description": "desc", "type": "promptString"}]
    )
    vscode_task_runner_old.variables.questionary.text.assert_called_once_with(
        "desc", default=""
    )
...
def test_get_input_value_password(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch.object(
        vscode_task_runner_old.variables.questionary, attribute="password"
    )
    ...
    vscode_task_runner_old.variables.questionary.password.assert_called_once_with(
        "desc", default=""
    )
...
def test_get_input_value_select(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch.object(
        vscode_task_runner_old.variables.questionary, attribute="select"
    )
    ...
    vscode_task_runner_old.variables.questionary.select.assert_called_once_with(
...
def test_get_input_variables_values(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch.object(
        vscode_task_runner_old.variables, attribute="get_input_value", return_value=None
    )
    ...
    vscode_task_runner_old.variables.get_input_variables_values(commands)

    # ensure only two calls were made
    # set does not maintain order
    vscode_task_runner_old.variables.get_input_value.assert_has_calls(
...
def test_replace_env_variables(environment_variable: None) -> None:
    ...
    """
    assert (
        vscode_task_runner_old.variables.replace_env_variables(
...
def test_replace_static_variables(environment_variable: None) -> None:
    ...
    assert (
        vscode_task_runner_old.variables.replace_static_variables(input_data)
        == output_data
    )
...
def test_replace_static_variables_unsupported() -> None:
    ...
    with pytest.raises(UnsupportedVariable):
        vscode_task_runner_old.variables.replace_static_variables(
            {"options": "${lineNumber}"}
        )
...
def test_replace_runtime_variables() -> None:
    ...
    assert (
        vscode_task_runner_old.variables.replace_runtime_variables(
