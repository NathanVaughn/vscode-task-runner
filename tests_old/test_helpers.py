...
from vscode_task_runner_old.exceptions import FileNotFound, InvalidValue
...
def test_identify_shell_type(
    ...
    assert (
        vscode_task_runner_old.helpers.identify_shell_type(shell_executable)
        == shell_type
    )
...
def test_identify_shell_type_fail() -> None:
    ...
    with pytest.raises(FileNotFound):
        vscode_task_runner_old.helpers.identify_shell_type("/not/real")
...
def test_stringify_pass(input_: Any, output: str) -> None:
    ...
    assert vscode_task_runner_old.helpers.stringify(input_) == output
...
def test_stringify_fail(input_: Any, exception: Type[Exception]) -> None:
    ...
    with pytest.raises(exception):
        vscode_task_runner_old.helpers.stringify(input_)
...
def test_combine_string(input_: Any, output: str) -> None:
    assert vscode_task_runner_old.helpers.combine_string(input_) == output
...
def test_load_command_string_pass(input_: Any, output: CommandString) -> None:
    assert vscode_task_runner_old.helpers.load_command_string(input_) == output
...
def test_load_command_string_fail(input_: Any, exception: Type[Exception]) -> None:
    with pytest.raises(exception):
        vscode_task_runner_old.helpers.load_command_string(input_)
